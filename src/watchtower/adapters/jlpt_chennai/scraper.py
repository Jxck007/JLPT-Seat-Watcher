...
from watchtower.adapters.jlpt_chennai.parser import (
    ParsedSeat,
    ParserError,
    parse_levels,
)
...
class Scraper:
    ...
    def _rendered(self) -> tuple[tuple[ParsedSeat, ...], float]:
        started = time.monotonic()
        html = ""
        browser: Any = None
        page: Any = None
    ...
    def fetch_all(self) -> tuple[SeatObservation, ...]:
        """Fetch once and parse every configured level."""
        ...
        try:
            ...
            method = "requests"
            if any(item.structure_changed for item in parsed):
                self._save_html(
        ...
        return tuple(
            SeatObservation(
                ...
                fetch_method=method,
                structure_changed=item.structure_changed,
            )
            for item in parsed
        )

    def fetch(self) -> SeatObservation:
        """Return the first configured observation through the legacy API."""
